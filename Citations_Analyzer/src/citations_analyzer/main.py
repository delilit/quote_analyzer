...
class CitationAnalyzer:
    ...
    def parse_doi_input(self, input_text: str, max_dois: int = 200) -> List[str]:
        ...
        cleaned_dois = [self.normalize_doi(doi) for doi in dois if self.validate_doi(self.normalize_doi(doi))]
            ...
            while citing_url:
                ...
                if response.status_code == 200:
                    ...
                    for work in data.get('results', []):
                        if work.get('doi'): citing_dois.append(self.normalize_doi(work['doi']))
